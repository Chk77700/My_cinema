 ...
 * @param $conn
 * @param $limit
 */  //function recherche de film sur l'index avec la pagination necessaire
function SearchFilm($film, $genre, $gj, $g, $distrib, $dj, $d, $date, $conn, $limit) {
	$film = TestWords($film, 'titre');
	$stmt = $conn->prepare("SELECT *$g$d FROM film $gj $dj WHERE (film.titre LIKE '%$film%') $genre $distrib $date"); //Declaration de la requete : Recherche par titre, distributeur, genre et date
	$stmt->execute();
	$rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
	$nb_page = ceil(count($rows) / $limit); // calcule le nombre de page necessaire pour le nombre de film qu'il y'a
	$page = ($_GET["page"] - 1) * $limit;
	$stmt = $conn->prepare("SELECT *$g$d FROM film $gj $dj WHERE (film.titre LIKE '%$film%') $genre $distrib $date ORDER BY film.annee_prod DESC, film.titre LIMIT $page, $limit");
	$stmt->execute();
	$rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
	HtmlInput($rows);
	if ($nb_page > 1) {
...
function HtmlPage($nb_page, $limit) {
    ...
    <a href='index.php?page=$precedent&film=$film$genre$distrib$date&limit=$limit'>&lsaquo;</a>
    ";
	for ($i = 1; $i <= $nb_page; $i++) {
		if ($i == $_GET["page"]) {
			$nombre_page = "<u style=\"color: orange;\">$i</u>";
		} else {
			$nombre_page = $i;
		}
		echo "
        <a href='index.php?page=$i&film=$film$genre$distrib$date&limit=$limit'>$nombre_page</a>
        ";
	}
	echo "
    <a href='index.php?page=$suivant&film=$film$genre$distrib$date&limit=$limit'>&rsaquo;</a>
    </div>
    </div>
    ";
}
...
function HtmlInput($rows) {
    /** @var TYPE_NAME $regex */
    $regex = "/(src=\")(.*?)(\" width)/"; //regex pour l'image source de l'api
	echo "
    <main>
    ";
	foreach ($rows as $row) {
		...
		if (substr($row['resum'], -1) != '.') {
			$row['resum'] .= '...<br/><a href="#">Lire la suite</a>';
		}
		...
		echo "<div class='film'>
        <h2>" . $row["titre"] . "<br/></h2>
        <img class='api' value='" . $row["titre"] . "' id='" . $row["titre"] . "'><br/>
